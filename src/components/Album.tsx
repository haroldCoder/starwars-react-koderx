...
export default function Album({laminas}: {laminas: {
  ...
  return (
    <div className={`flex ${laminas.length == 0 ? 'justify-center items-center p-20' : 'p-10'}`}>
      {
        laminas.length == 0 ? /* si no hay laminas almacenadas, mostrar imagen de 404 */
        ...
        <section className="gap-y-10 flex flex-col">
          <div>
            <h2 className="text-3xl text-gray-600 font-semibold">Peliculas</h2>
            {
              laminas.map((e)=>(
                e.movies.length != 0 ? 
                  e.movies.map((mv)=>( /* hacer doble mapeo, para acceder a un item especifico */
